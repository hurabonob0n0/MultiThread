...
public:
	...
	NODE* get_node(int value) {
		if (get_pool.empty()) {
			...
			node->data = value;
			node->next = nullptr;
			node->removed = false; // Reset the removed flag when reusing the node
			return node;
		}
	}
...
public:
	...
	bool Remove(int x)
	{
		while (true) {
			...
			if (curr->data != x) {
				...
				pred->next = curr->next; // Physically remove the node from the list
				pred->unlock(); curr->unlock(); // Unlock the mutex after modifying the list
				memory_pool[thread_id].free_node(curr); // Recycle the removed node back to the memory pool
				return true; // Element added successfully
			}
	...
	bool Contains(int x)
	{
		...
		// 노드가 리스트에 있고, removed 플래그가 false인지 확인 (Lock 없이!)
