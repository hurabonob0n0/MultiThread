...
public:
	ZLIST_ASP()
	{
		auto h = std::make_shared<NODE_ASP>(std::numeric_limits<int>::min());
		auto t = std::make_shared<NODE_ASP>(std::numeric_limits<int>::max());
		h->next.store(t);
		head.store(h);
		tail.store(t);
	}
	...
	bool validate(std::shared_ptr<NODE_ASP> pred, std::shared_ptr<NODE_ASP> curr)
	{
		return (!pred->removed) && (!curr->removed)
			&& (pred->next.load() == curr);
	}
	...
	bool Add(int x)
	{
		while (true) {
			...
			if (curr->data == x) {
				...
				auto new_node = std::make_shared<NODE_ASP>(x);
	...
	bool Contains(int x)
	{
		std::shared_ptr<NODE_ASP> curr = head.load();
		while (curr->data < x) {
			curr = curr->next.load();
		}
		return (curr->data == x) && (!curr->removed);
	}
